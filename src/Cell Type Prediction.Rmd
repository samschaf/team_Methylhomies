...
```{r}
load("GSE59685_filtered_brain.RData") # load normalized 450K data
dim(GSE59685_filtered_brain)
...
```{r}
prop <- estProportion(GSE59685_filtered_brain, profile = refProfile)
prop<-as.data.frame(prop)
prop$glia<-apply(prop,1,function(x) 1-x)
colnames(prop)<- c("neuron", "glia")
head(prop)
write.csv(prop, file = "Hannon_Neuronal_Glia_Proportions.csv", row.names=T)
summary(prop)
plot(density(prop$neuron), main="Neuronal Proportion Density") 
```

There seems to be four peaks here - likely corresponding to four brain regions, each with its own unique neuron to glia ratio.